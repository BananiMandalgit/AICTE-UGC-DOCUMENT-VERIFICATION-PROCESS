...
export function EvaluatorLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const token = await api.post(`/evaluator/auth/login`, {
        authKey: email,
        password: password,
      });
      if (token.data?.data?.token) {
        console.log(token);
        setTokenMode(token.data.data.token, "evaluator");
      ...
      <div className="max-w-4xl w-full space-y-8 p-10 bg-white rounded-xl shadow-2xl">
        <div className="flex flex-col md:flex-row items-center justify-between">
          <div className="w-full md:w-1/2 pr-8">
            <img src="/approval-enginex-logo.png" alt="Approval Enginex Logo" className="mx-auto mb-8" />
            ...
            <div className="space-y-4 text-sm">
              <div className="flex items-center">
                <CheckCircle2 className="h-5 w-5 mr-2 text-[#0b6e4f]" />
            ...
            <Card className="w-full">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <CardFooter>
                  <Button
                    type="submit"
                    className="w-full bg-[#0b6e4f] hover:bg-[#095a40]"
                  >
                    Sign In
                  </Button>
                </CardFooter>
              ...
              <div className="text-center mt-4 text-sm">
                <a href="#" className="text-[#0b6e4f] hover:underline">
                  Forgot password?
                </a>
              </div>
            </Card>
