...
const getLatestDocuments = (application: UniversityApplication): ApplicationDocument[] => {
  const latestDocs = new Map<string, ApplicationDocument>();

  (application.UniversityDocuments || []).forEach((doc) => {
    const existing = latestDocs.get(doc.doc_id);
    if (!existing) {
      latestDocs.set(doc.doc_id, doc);
      return;
    }

    const existingTime = new Date(existing.timestamp).getTime();
    const currentTime = new Date(doc.timestamp).getTime();
...
const getApplicationStatus = (application: UniversityApplication): string => {
  // ✅ FIX: Use application-level status field (from backend state machine)
  // This now reflects: NOT_SUBMITTED → SUBMITTED → IN_REVIEW → APPROVED/REJECTED
  ...
  // ✅ Fallback to document-based status (for backward compatibility)
  const documents = getLatestDocuments(application);
  const expectedDocs = application.application?.documents?.length ?? documents.length;

  if (!documents.length && expectedDocs === 0) {
    return "Not Started";
  }

  const decisionStatuses = documents.map((doc) => doc.evaluatorDecision?.status || doc.status);

  if (decisionStatuses.some((status) => status === "REJECTED")) {
    return "Rejected";
  }

  const approvedCount = decisionStatuses.filter((status) => status === "APPROVED").length;
  if (expectedDocs > 0 && approvedCount >= expectedDocs) {
    return "Approved";
  }

  if (documents.some((doc) => doc.status !== "NOT_SUBMITTED")) {
    return "In Progress";
  }
...
const ApplicationsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      try {
        const data = await getApplication(instituteId);
                    ...
                    >
                      <div className="flex-1 space-y-1">
                        <p className="text-sm font-medium leading-none text-[#2c3e50]">
                          {app.application_name || app.application.application_name}
                        </p>
                        <p className="text-sm text-[#7f8c8d]">
                          {app.application.application_name} • {app.submittedAt 
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="space-y-2">
                        {["Approved", "In Progress", "Not Started", "Rejected"].map(
                          (status) => (
                ...
                filteredApplications.map((app) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <Avatar>
                        <AvatarFallback>
                          {(app.application_name || app.application.application_name).charAt(0)}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="text-sm font-medium text-[#2c3e50]">
                          {app.application_name || "Untitled Application"}
                        </p>
                        <p className="text-sm text-[#7f8c8d]">
                          {app.application.application_name} • {app.submittedAt
                            ? new Date(app.submittedAt).toDateString()
                            : (app.createdOn ? new Date(app.createdOn).toDateString() : "")}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
