...
export const FacultyScoreValidation: React.FC = () => {
  const { toast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [file, setFile] = useState<File | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [analysis, setAnalysis] = useState<FacultyAnalysis | null>(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/validate-faculty-credentials", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.success) {
        ...
        toast({
          description: "Faculty credentials validated successfully",
        });
      } else {
        throw new Error("Validation returned no data");
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "An error occurred during validation. Please ensure the backend server is running on http://localhost:8000";
      ...
      <CardHeader>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <CardTitle className="text-2xl text-slate-900">Faculty Score Validation</CardTitle>
            <CardDescription className="text-slate-600 mt-2">
              Upload faculty credentials PDF and get AI-powered validation report using Llama model analysis
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        {!analysis && !error && (
          <div className="space-y-4">
            <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center bg-slate-50 hover:bg-slate-100 transition">
              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf"
              ...
              <p className="text-slate-900 font-medium mb-2">
                {fileName ? `Selected: ${fileName}` : "Upload Faculty Credentials PDF"}
              </p>
              <p className="text-slate-500 text-sm mb-4">
                Click below or drag and drop your PDF file (Max 10MB)
              </p>
              <Button
                variant="outline"
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-slate-600 font-medium">Processing faculty credentials with Llama AI...</p>
            <p className="text-slate-500 text-sm">This may take a moment depending on document size</p>
          </div>
        )}
              ...
              <div className="text-center">
                ...
                <div className="flex justify-center gap-3 flex-wrap">
                  <Badge className={`${getStatusColor(analysis.validation_status)} border`}>
                    {analysis.validation_status === "VALID" && <CheckCircle className="w-3 h-3 mr-1" />}
                    {analysis.validation_status}
                  </Badge>
...
export default FacultyScoreValidation;
