...
import { FileText, Home, ChevronLeft, ChevronRight, Search, MessageSquare } from 'lucide-react'
...
export const CollapsibleSidebar: React.FC<CollapsibleSidebarProps> = ({ onCollapse }) => {
  ...
  const loginSource = typeof window !== 'undefined' ? localStorage.getItem('loginSource') : 'aicte';
  return (
    <motion.div
      className={cn(
        ...
        <AnimatePresence>
          {!isCollapsed && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <img src="/approval-enginex-logo.png" alt="Approval Enginex Logo" className="w-8 h-8" />
              <h1 className="font-semibold text-lg">{loginSource === 'ugc' ? 'UGC Portal' : 'AICTE Portal'}</h1>
            </motion.div>
          )}
        </AnimatePresence>
        <motion.div
                  ...
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Button
                        variant="ghost"
                        className={cn(
                          'w-full justify-start h-10 transition-colors hover:bg-muted/50',
                          isCollapsed ? 'px-2' : 'px-4'
                        )}
                        onClick={() => navigate(-1)}
                      >
                        <ChevronLeft className={cn('h-5 w-5 shrink-0', isCollapsed ? 'mr-0' : 'mr-3')} />
                  ...
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Button
                        variant="ghost"
                        className={cn(
                          'w-full justify-start h-10 transition-colors hover:bg-muted/50 hover:text-red-600',
                          isCollapsed ? 'px-2' : 'px-4'
                        )}
                        onClick={() => {
